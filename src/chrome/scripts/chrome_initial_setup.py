...
def chrome_initial_setup(profile_name: str | int, script_data_path: str, driver: webdriver.Chrome) -> None:
    """
    Первичная настройка профиля Chrome
    ...
    """
    logger.info(f"🔧 {profile_name} - Начинаю первичную настройку Chrome")
    
    # Создаем окно прогресса
    progress = QProgressDialog()
    progress.setWindowTitle("Первичная настройка Chrome")
    progress.setLabelText(f"Подготовка к настройке профиля {profile_name}...")
    progress.setMinimum(0)
    progress.setMaximum(5)  # 5 шагов: загрузка конфига + 4 настройки
    ...
    try:
        ...
        try:
            with open(os.path.join(script_data_path, 'config.json'), 'r', encoding="utf-8") as f:
                config = json.load(f)
                logger.info(f"📋 {profile_name} - Загружена конфигурация из {script_data_path}")
                results["config"] = True
                progress.setValue(1)
        except Exception as e:
            logger.error(f"❌ {profile_name} - Ошибка загрузки конфигурации: {str(e)}")
            raise

        working_tab = driver.current_window_handle

        if config["run_delay_sec"]:
            logger.debug(f"⏳ {profile_name} - Ожидание {config['run_delay_sec']} сек")
            time.sleep(config["run_delay_sec"])

        close_all_other_tabs(driver, working_tab)

        # Отключаем синхронизацию
        progress.setLabelText(f"Профиль {profile_name}\nОтключение синхронизации...")
        try:
            turn_off_sync(profile_name, driver, working_tab)
            results["sync"] = True
            progress.setValue(2)
        except Exception as e:
            logger.error(f"❌ {profile_name} - Ошибка отключения синхронизации: {str(e)}")

        # Отключаем автозаполнение
        progress.setLabelText(f"Профиль {profile_name}\nОтключение автозаполнения...")
        try:
            turn_off_autofill(profile_name, driver, working_tab)
            results["autofill"] = True
            progress.setValue(3)
        except Exception as e:
            logger.error(f"❌ {profile_name} - Ошибка отключения автозаполнения: {str(e)}")
        ...
        try:
            ...
            logger.error(f"❌ {profile_name} - Ошибка настройки приватности: {str(e)}")

        # Настраиваем запоминание вкладок
        progress.setLabelText(f"Профиль {profile_name}\nНастройка запоминания вкладок...")
        try:
            remember_tabs = config["startup_settings"]["remember_tabs"]["value"]
            adjust_tabs_memorizing(profile_name, driver, working_tab, remember_tabs)
            results["tabs"] = True
            progress.setValue(5)
            logger.info(f"✅ {profile_name} - Запоминание вкладок {'включено' if remember_tabs else 'выключено'}")
        except Exception as e:
            logger.error(f"❌ {profile_name} - Ошибка настройки запоминания вкладок: {str(e)}")

        # Выводим итоговый результат
        success_count = sum(1 for v in results.values() if v)
        if success_count == len(results):
            logger.info(f"✨ {profile_name} - Первичная настройка Chrome успешно завершена")
            progress.setLabelText(f"Профиль {profile_name}\nВсе настройки успешно применены!")
        else:
            failed = [k for k, v in results.items() if not v]
            logger.warning(f"⚠️ {profile_name} - Настройка завершена частично. Не удалось: {', '.join(failed)}")
            progress.setLabelText(f"Профиль {profile_name}\nЧастично применены настройки.\nНе удалось: {', '.join(failed)}")
        
        # Даем пользователю время прочитать результат
        time.sleep(2)
        
    except Exception as e:
        logger.error(f"❌ {profile_name} - Ошибка при первичной настройке Chrome: {str(e)}")
